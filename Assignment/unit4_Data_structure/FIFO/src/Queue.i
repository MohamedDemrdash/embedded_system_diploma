...
FIFO_state FIFO_init (FIFO_buffer* fifo_buf,uint8* arr,uint16 length);
FIFO_state FIFO_enqueue (FIFO_buffer* fifo_buf,uint8 item);
FIFO_state FIFO_dequeue (FIFO_buffer* fifo_buf,uint8* item);
...
int main(void) {
 ...
 uint16 op;
 uint8 UART_base_arr[5]; FIFO_buffer UART_fifo; FIFO_init(&UART_fifo,UART_base_arr,5);
 while(1)
 {
...
# 31 ".\\Queue.c"
   ...
   scanf("%hu",&item);
   FIFO_add(&UART_fifo,item);
   break;
  case 2:
   printf("Remove an item\n");
   FIFO_get(&UART_fifo,&item);
   printf("%d\n",item);
   break;
  case 3:
